...
const Navbar = () => {
    ...
    // for when login userprofile shown on the button
    const [showMenu, setShowMenu] = useState(false)
    const [token, setToken] = useState(true) //when there's token,login otherwise not login

  return (
    <div className='flex item-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
        <img onClick={()=>navigate('/')} className='w-44 cursor-pointer' src={assets.logo} alt="" srcset="" />
